...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KyZXEAg3QhqLMpG8r+8fhAXLRk2vvoC2f3B09zVXn8CA5QIVfZOJ3BCsw2P0p/We" crossorigin="anonymous">
    ...
    background-image: url('/images-p/image5.jpg');
                 ...
                 <table class="table table-striped table-inverse table-responsive">
                     <thead class="thead-inverse">
                         <tr>
                             ...
                             <th>Email</th>
                             <th>Diseases Type</th>
                             <th>Diseases Level</th>
                             <th>Action</th>
                         </tr>
                         </thead>
                         <tbody>
                         <tr>
                                 <td scope="row">{{ Auth::guard('student')->user()->fullname }}</td>
                                 <td>{{ Auth::guard('student')->user()->address }}</td>
                                 <td>{{ Auth::guard('student')->user()->age }}</td>
                                 <td>{{ Auth::guard('student')->user()->dob }}</td>
                                 <td>{{ Auth::guard('student')->user()->gender }}</td>
                                 <td>{{ Auth::guard('student')->user()->email }}</td>
                                 <td>{{ Auth::guard('student')->user()->diseasestype }}</td>
                                 <td>{{ Auth::guard('student')->user()->diseaseslevel }}</td>
                                 <td>
